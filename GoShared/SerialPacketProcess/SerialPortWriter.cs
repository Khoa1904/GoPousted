...
{
    private static readonly Lazy<SerialPortWriter> Lazy = new(() => new SerialPortWriter());
    ...
    public static SerialPortWriter Go => Lazy.Value;
    ...
    */
    public void Open(string portname = "COM3", int baudrate = 9600, Parity parity = Parity.None, int databits = 8, StopBits stopbits = StopBits.One, Handshake handshake = Handshake.None)
    {
        Close();
        try
        {
    ...
    public void SendString(string message)
    {
        ...
        try
        {
            ...
            byte[] bytes = Encoding.GetEncoding(51949).GetBytes(message);
            _serialPort.Write(bytes, 0, bytes.Length);
            OnStatusChanged?.Invoke(this, $"Message sent Hex : {TypeHelper.StringToHexString(message).LogEncrypt()}");
        }
        catch (Exception ex)
        {
            OnStatusChanged?.Invoke(this, $"Failed to send string(Printer not working) : {ex.Message}");
        }
    }
